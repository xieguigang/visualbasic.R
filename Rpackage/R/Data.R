#Region "Microsoft.ROpen::cb3e410bfd0bf115dc2a6a51db216f3d, Data.R"
    ...
    # data.frame.empty = function(nrow, ncol, colnames = NULL, rownames = NULL) {...
...
Microsoft.VisualBasic.Data <- function() {
  ...
  data.frame.empty = function(nrow, ncol, colnames = NULL, rownames = NULL) {
    data <- data.frame(matrix(ncol = nrow, nrow = ncol));
  ...
  list(namespace = GetCurrentFunc(),
       ...
       methods = list(
         ...
         data.frame.empty = data.frame.empty
       ));
}
